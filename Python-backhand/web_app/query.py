...
SELECT
    ...
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '09:00:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '10:00:00'
        THEN 1 ELSE 0
    END), 0) AS pass_9_10,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '10:00:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '11:00:00'
        THEN 1 ELSE 0
    END), 0) AS pass_10_11,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '11:00:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '12:15:00'
        THEN 1 ELSE 0
    END), 0) AS pass_11_1215,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '12:15:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '13:45:00'
        THEN 1 ELSE 0
    END), 0) AS pass_1215_145,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '13:45:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '14:45:00'
        THEN 1 ELSE 0
    END), 0) AS pass_145_245,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '14:45:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '16:00:00'
        THEN 1 ELSE 0
    END), 0) AS pass_245_4,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '16:00:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '17:00:00'
        THEN 1 ELSE 0
    END), 0) AS pass_4_5,
    COALESCE(SUM(CASE
        WHEN checker_output.field_name = 'pass'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) >= '17:00:00'
            AND TIME(COALESCE(checker_output.actual_event_time, checker_output.recorded_at)) < '18:00:00'
        THEN 1 ELSE 0
    END), 0) AS pass_5_6
FROM checker_output
JOIN po
